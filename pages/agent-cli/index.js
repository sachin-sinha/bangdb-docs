    ...
    <React.Fragment>
      ...
      <DocArticle>
        ...
        <div className="article-body">
          <p>
            Agent is used for streaming data into BangDB. It runs as a service
            on the server from where data is supposed to be streamed. Data could
            be streamed in continuous basis or in batch manner. The agent can
            also parse and structure data before sending to BangDB if
            configured. It can parse csv, key-val data and structure them into
            json or it can also use regex if supplied to structure the data.
          </p>
          <p>
            Further, it also uses inbuilt default logic to parse and structure
            known data entities. And finally, we can also use IE (information
            extraction) to do NER (name entity recognition) to structure the
            data in probabilistic manner. However, the goal is to send data to
            BangDB structured or otherwise.
          </p>
          <p>
            Agent could read data from a log file in continuous manner. Or it
            can read from other services using API or otherwise. It can also
            read using protocol like SMTP etc. Idea is to continuously keep
            reading data, in an eventing manner.
          </p>
          <p>
            Agent can be queried or instructed remotely using cli. Following are
...
stats agent collector_name where server = IP:PORT`}
          ...
          <p>
            ...
            Let&apos;s say that agent is running on a server and we wish to
            instruct it to start collecting data from new file or source with a
            given configuration. We can do this by calling
          ...
          />
          <p>
            To see the details of any agent, including what all collectors are
            registered, activated, paused etc.
          </p>
          <HighlightedCpp
