      ...
      <DocArticle>
        ...
        <div className="article-body">
          ...
          <ul>
            ...
            <li>
              <strong>Automatically identify issues:</strong>one of the benefits
              of ML is that it detects issues and problems automatically, even
              if there’s a huge number of logs.
            </li>
            <li>
          ...
          <p>
            ...
            data across and on- premises environments. They Provide data space
            or volumes to their users. They have space containers and every The
            container has some fixed space. Each container has many clusters.
            Their users are grouped in these clusters, every cluster has some
            fixed memory allotted to it. They wanted a solution that can monitor
            the data usage pattern for users and can predict when the space
            allotted will be full so that they can increase storage before it
            goes down due to memory shortage.
          </p>
          <h3>The Problem</h3>
          <p>
          ...
          <p>
            ...
            growth rate percentage and few other attributes.
          </p>
          ...
          Adding filter to collect details for aggregate with usage above 85 and
          adding groupby to monitor their average usage. Adding filter in
          ...
          <ul>
            ...
            <li>
              ...
              workflow for ml training, the user just have to enter the training
              details
            </li>
            <li>
            ...
            <li>
              what's the position of timestamp field in the csv (count starts
              with 0): 0 [provide the position of date/time attribute in the
              training file]
              <li>
                enter the time-stamp field datatype [ string (5) | long (9) |
                double (11) ] (or Enter for default (9)): 5 [here, we have to
                tell whether time attribute is in which format--if any date
                format the select 5, if in timestamp then select 11/9]
              </li>
              <li>
                what's the position of entity field in the csv (count starts
                with 0, enter -1 ignore): 4 [As we have many aggregates]
              </li>
              <li>
                what's the position of quantity field in the csv (count starts
                with 0): 6 [here, used capacity will be target, provide the
                position of this attribute in training file]
              </li>
              <i>
                Do you wish to aggr for a higher time dimension? [ yes | no ]:
                yes [As we have to perform aggregation day wise]
              </i>
              <li>
                what's the lag for data to be selected for prediction (default
                is 2)?: 6 [to predict next day volume, taking the last 6 days
                used capacity values. In which present value will be selected as
                target for training]
              </li>
              <li>
                what's the offset for data to be selected for prediction
                (default is 0)?: 0 [As we are predicting next day's values]
              </li>
              <strong>Once we are done, we get forecast train request :</strong>
              <p></p>
              <img
